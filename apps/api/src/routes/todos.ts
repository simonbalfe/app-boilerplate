...
export const todoRoutes = new Hono()
  .get(
    ...
    async (c) => {
      ...
      return c.json({ success: true as const, todos: listTodos() })
    },
  )
  .post(
    '/todos',
    ...
    async (c) => {
      ...
      if (!text || typeof text !== 'string' || text.trim().length === 0) {
        return c.json({ success: false as const, error: 'Todo text is required' }, 400)
      }

      if (text.length > 200) {
        return c.json(
          { success: false as const, error: 'Todo text must be less than 200 characters' },
          400,
        )
      }

      await new Promise((resolve) => setTimeout(resolve, 300))
      return c.json({ success: true as const, todo: createTodo(text) })
    },
  )
  .post(
    '/todos/delete',
    ...
    async (c) => {
      ...
      if (!id || typeof id !== 'string') {
        return c.json({ success: false as const, error: 'Todo ID is required' }, 400)
      }
  ...
  .post(
    ...
    async (c) => {
      ...
      return c.json({ success: true as const, todo })
    },
  )
